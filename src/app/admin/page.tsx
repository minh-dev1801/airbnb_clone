...
const StatsCards = dynamic(
  () => import('@/app/components/admin/dashboard/StatsCards/StatsCards'),
  {
    ssr: false,
  }
);
...
const Dashboard: React.FC = () => {
    ...
    return (
      ...
      if (key === 'bookings') {
        return bookings.filter(
          (b) =>
            dayjs(b.ngayDen).isAfter(monthsAgo) ||
            dayjs(b.ngayDi).isAfter(monthsAgo)
        ).length;
      }
      ...
      if (key === 'checkIns') {
        return bookings.filter((b) => dayjs(b.ngayDen).isAfter(monthsAgo))
          .length;
      }

      if (key === 'checkOuts') {
        return bookings.filter((b) => dayjs(b.ngayDi).isAfter(monthsAgo))
          .length;
      }
  ...
  const stats = useMemo(
    () => ({
      ...
      recentCheckOuts: calculateStat(
        timeRanges.checkOuts,
        'checkOuts',
        bookings
      ),
    }),
  ...
  const mockData: MockDataType = useMemo(
    () => ({
      '2023': {
